...
- Working plan docs are the record of project state (rebaseline history, open bugs): `iridescent_dish_nextsteps.md` (active feature plan), `anisotropy_nextsteps.md` (2026-09-11 — AnisotropyBarnLamp ingest; aniso GGX + emissive-strength + clearcoat (direct+IBL+clearcoatNormal, both backends) done; GLB reader + env-background seam open), `glass_parity_nextsteps.md` (closed 2026-09-05 — see "Open items"), `dragon_nextsteps.md`, `nextsteps.md` (lamp history). See README "Next Steps". Note `nextsteps.md` has drifted: its line-number citations are stale and some items listed as deferred are already done — verify against HEAD.
